...
export default function IssueDetail() {
  ...
  useEffect(() => {
    if (id && id !== 'undefined') {
      ...
      setError(t('issueDetail.invalidId', 'Invalid issue ID')); // ADDED
      setLoading(false);
    }
  }, [id]);
  ...
  const fetchIssue = async () => {
    try {
      ...
      console.error("Error fetching issue:", err);
      setError(t('issueDetail.loadError', "Failed to load issue details.")); // UPDATED
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpvote = async () => {
    if (!user) {
      alert(t('issueDetail.loginToVote')); // UPDATED
      navigate("/login");
      return;
    }
    ...
    try {
      ...
      console.error("Error voting:", err);
      alert(err.message || t('issueDetail.voteError', "Failed to register vote")); // UPDATED
    }
  };
  ...
  const handleCommentSubmit = async (e) => {
    ...
    if (!user) {
      alert(t('issueDetail.loginToComment')); // UPDATED
      navigate("/login");
      return;
    }
    ...
    try {
      ...
      console.error("Error posting comment:", err);
      alert(err.message || t('issueDetail.commentError', "Failed to post comment")); // UPDATED
    } finally {
      setIsSubmittingComment(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-tr from-[#43c6ac] to-[#191654] flex items-center justify-center">
        <div className="text-white text-center">
          <div className="animate-spin rounded-full h-16 w-16 border-b-2 border-white mx-auto mb-4"></div>
          <p className="text-lg">{t('issueDetail.loadingDetails')}</p>
        </div>
      </div>
    );
  }
  ...
  if (error || !issue) {
    return (
      <div className="min-h-screen bg-gradient-to-tr from-[#43c6ac] to-[#191654] flex items-center justify-center">
        <div className="bg-white/15 backdrop-blur-md rounded-2xl p-8 text-center border border-white/20 max-w-md">
          <h2 className="text-2xl font-semibold text-white mb-4">{t('issueDetail.notFound')}</h2>
          <p className="text-gray-200 mb-6">{error || t('issueDetail.notFoundDesc')}</p>
          <button
            onClick={() => navigate('/issues')}
            className="bg-gradient-to-b from-[#00b4db] to-[#0083b0] text-white px-6 py-3 rounded-full font-semibold hover:scale-105 transition-transform"
          >
    ...
    <div className="min-h-screen bg-gradient-to-tr from-[#43c6ac] to-[#191654] py-8 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <button
          ...
          className="flex items-center gap-2 text-white mb-6 hover:text-cyan-300 transition-colors"
        >
          <FaArrowLeft /> {t('issueDetail.back')}
        </button>
        ...
        <div className="bg-white/15 backdrop-blur-md rounded-2xl border border-white/20 overflow-hidden">
          ...
          <div className="p-6 border-b border-white/20">
            <div className="flex items-start justify-between mb-4">
              <div>
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <span className={`${getStatusClass(issue.status)} text-white px-3 py-1 rounded-full text-sm font-medium capitalize`}>
                    {t(`status.${issue.status?.toLowerCase()?.replace(' ', '')}`, issue.status || 'Open')}
                  </span>
                  <div className="flex items-center gap-1.5 bg-white/20 text-white rounded-full px-3 py-1">
            ...
            <div className="flex flex-wrap gap-4 text-white/80 text-sm">
              ...
              <div className="flex items-center gap-2">
                <FaClock />
                <span>{t('issueDetail.reportedOn')} {formatDate(issue.reportedDate || issue.createdAt)}</span>
              </div>
              <div className="flex items-center gap-2">
                <FaUser />
                <span>{t('issues.reportedBy')} {issue.reporterName || t('common.anonymous', 'Anonymous')}</span>
              </div>
            </div>
          </div>
          ...
          {/* Description */}
          <div className="p-6 border-b border-white/20">
            <h2 className="text-xl font-semibold text-white mb-3">{t('issueDetail.description')}</h2>
            <p className="text-white/90 leading-relaxed whitespace-pre-wrap">
              {issue.description}
            </p>
          </div>
          ...
          <div className="p-6 border-b border-white/20">
            <div className="flex items-center gap-6">
              <button
                ...
                title={!user ? t('issueDetail.loginToVote') : hasUpvoted ? t('issueDetail.removeUpvote') : t('issueDetail.upvote')}
              >
                <FaThumbsUp />
                <span className="font-semibold">{issue.upvotes || 0}</span>
              </button>
              ...
              <button
                ...
                title={!user ? t('issueDetail.loginToVote') : hasDownvoted ? t('issueDetail.removeDownvote') : t('issueDetail.downvote')}
              >
                <FaThumbsDown />
                <span className="font-semibold">{issue.downvotes || 0}</span>
              </button>
              ...
              <div className="flex items-center gap-2 text-white/80">
                <FaCommentDots />
                <span>{Array.isArray(issue.comments) ? issue.comments.length : 0} {t('issues.comments')}</span>
              </div>
            </div>
          </div>
          ...
          {/* Comments Section */}
          <div className="p-6">
            <h2 className="text-xl font-semibold text-white mb-4">{t('issues.comments')}</h2>
              ...
              <form onSubmit={handleCommentSubmit} className="mb-6">
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-b from-[#00b4db] to-[#0083b0] text-white px-6 py-2 rounded-full font-semibold hover:scale-105 transition-transform disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isSubmittingComment ? t('issueDetail.posting') : t('issueDetail.postComment')}
                  </button>
                </div>
              </form>
            ) : (
              <div className="bg-white/10 rounded-lg p-4 mb-6 text-white/80 text-center">
                <p>{t('issueDetail.loginToComment')}</p>
              </div>
            )}
              ...
              {Array.isArray(issue.comments) && issue.comments.length > 0 ? (
                issue.comments.map((comment) => (
                  <div key={comment._id || comment.id} className="bg-white/10 rounded-lg p-4">
                    <div className="flex items-center gap-2 mb-2">
                      <FaUser className="text-white/60" />
                      <span className="text-white font-medium">{comment.userName || t('common.anonymous', 'Anonymous')}</span>
                      <span className="text-white/60 text-sm">{formatDate(comment.createdAt)}</span>
                    </div>
                    <p className="text-white/90">{comment.text}</p>
                  </div>
                ...
                <p className="text-white/60 text-center py-8">{t('issueDetail.noComments')}</p>
              )}
            </div>
          </div>
        </div>
