...
export default function AdminUsers() {
  ...
  const fetchData = async () => {
    try {
      ...
      alert(t('admin.loadError', 'Failed to load users data')); // UPDATED
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      alert(t('admin.userUpdated', 'User updated successfully')); // UPDATED
    } catch (error) {
      console.error('Error updating user:', error);
      alert(error.message || t('admin.updateUserError', 'Failed to update user')); // UPDATED
    }
  };

  const handleDeleteUser = async (userId, userName) => {
    if (!window.confirm(t('admin.confirmDeleteUser', `Are you sure you want to delete user "${userName}"? This action cannot be undone.`))) { // UPDATED
      return;
    }
    ...
    try {
      ...
      alert(t('admin.userDeleted', 'User deleted successfully')); // UPDATED
    } catch (error) {
      console.error('Error deleting user:', error);
      alert(error.message || t('admin.deleteUserError', 'Failed to delete user')); // UPDATED
    }
  };
      ...
      <div className="flex relative">
        ...
        <aside className={`${sidebarOpen ? 'w-64' : 'w-0'} min-h-screen bg-white/10 backdrop-blur-lg border-r border-white/20 overflow-hidden transition-all duration-300 ease-in-out`}>
          <div className="w-64 p-6 flex flex-col gap-4">
            ...
            <SidebarItem icon={BarChart2} label={t('admin.overview')} value="overview" active={activeTab === 'overview'} />
            <SidebarItem icon={FileText} label={t('admin.allIssues')} onClick={() => navigate('/issues')}  />
            <SidebarItem icon={Users} label={t('admin.users')} onClick={() => navigate('/admin/users')} />
            <SidebarItem icon={Download} label={t('admin.exportReports')} value="export" active={activeTab === 'export'} />
            
            <div className="border-t border-white/20 pt-4 space-y-2 mt-auto">
              <SidebarItem icon={Settings} label={t('admin.settings')} value="settings" active={activeTab === 'settings'} />
              <SidebarItem icon={LogOut} label={t('admin.logout')} value="logout" active={false} onClick={handleLogout} />
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 p-8 overflow-x-auto">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2">{t('admin.userManagement', 'User Management')}</h1>
            <p className="text-white/70">{t('admin.manageUsers', 'Manage and monitor all registered users')}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform">
              ...
              <div className="p-6 text-center">
                ...
                <div className="text-slate-600 font-medium">{t('admin.totalUsers', 'Total Users')}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform">
              ...
              <div className="p-6 text-center">
                ...
                <div className="text-slate-600 font-medium">{t('admin.adminUsers', 'Admin Users')}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform">
              ...
              <div className="p-6 text-center">
                ...
                <div className="text-slate-600 font-medium">{t('admin.regularUsers', 'Regular Users')}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:scale-105 transition-transform">
              ...
              <div className="p-6 text-center">
                ...
                <div className="text-slate-600 font-medium">{t('admin.activeUsers', 'Active Users')}</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 p-6 mb-6">
            <div className="flex flex-col md:flex-row gap-4">
              ...
              <div className="relative">
                ...
                >
                  <option value="all" className="bg-slate-800">{t('admin.allRoles', 'All Roles')}</option>
                  <option value="user" className="bg-slate-800">{t('admin.regularUsers', 'Regular Users')}</option>
                  <option value="admin" className="bg-slate-800">{t('admin.admins', 'Admins')}</option>
                </select>
              </div>
              ...
              >
                <option value="createdAt" className="bg-slate-800">{t('admin.sortByDate', 'Sort by Date')}</option>
                <option value="name" className="bg-slate-800">{t('admin.sortByName', 'Sort by Name')}</option>
                <option value="email" className="bg-slate-800">{t('admin.sortByEmail', 'Sort by Email')}</option>
                <option value="role" className="bg-slate-800">{t('admin.sortByRole', 'Sort by Role')}</option>
                <option value="reports" className="bg-slate-800">{t('admin.sortByReports', 'Sort by Reports')}</option>
              </select>
            </div>
          </div>
          ...
          <div className="bg-white/10 backdrop-blur-lg rounded-xl border border-white/20 overflow-hidden">
            {loading ? (
              <div className="text-center py-12">
                ...
                <p>{t('admin.loadingUsers', 'Loading users...')}</p>
              </div>
            ) : (
              <>
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead className="bg-white/5">
                      <tr>
                        <th className="px-6 py-4 text-left text-sm font-semibold">{t('admin.user', 'User')}</th>
                        <th className="px-6 py-4 text-left text-sm font-semibold">{t('auth.email', 'Email')}</th>
                        <th className="px-6 py-4 text-left text-sm font-semibold">{t('admin.role', 'Role')}</th>
                        <th className="px-6 py-4 text-center text-sm font-semibold">{t('admin.reports', 'Reports')}</th>
                        <th className="px-6 py-4 text-center text-sm font-semibold">{t('dashboard.resolved', 'Resolved')}</th>
                        <th className="px-6 py-4 text-left text-sm font-semibold">{t('admin.joined', 'Joined')}</th>
                        <th className="px-6 py-4 text-center text-sm font-semibold">{t('admin.actions', 'Actions')}</th>
                      </tr>
                    </thead>
                    <tbody>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    ...
                                    {isCurrentUser && (
                                      <span className="text-xs text-cyan-300">{t('admin.you', '(You)')}</span>
                                    )}
                                  </div>
                                </div>
                              </td>
                              ...
                              <td className="px-6 py-4">
                                <div className="flex items-center justify-center gap-2">
                                  <button
                                    ...
                                    title={isCurrentUser ? t('admin.cannotEditOwn', "Cannot edit your own account") : t('admin.editUser', "Edit user")}
                                  >
                                  ...
                                  <button
                                    ...
                                    title={isCurrentUser ? t('admin.cannotDeleteOwn', "Cannot delete your own account") : t('admin.deleteUser', "Delete user")}
                                  >
                ...
                {totalPages > 1 && (
                  <div className="flex items-center justify-between px-6 py-4 border-t border-white/10">
                    <div className="text-sm text-white/70">
                      {t('admin.showingUsers', `Showing ${startIndex + 1}-${Math.min(startIndex + itemsPerPage, filteredUsers.length)} of ${filteredUsers.length} users`)}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl border border-white/20 p-6 max-w-md w-full shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white">{t('admin.editUser')}</h2>
              <button
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-white text-sm font-semibold mb-2">{t('auth.name')}</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-white text-sm font-semibold mb-2">{t('auth.email')}</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-white text-sm font-semibold mb-2">{t('admin.role')}</label>
                <select
                ...
                >
                  <option value="user" className="bg-slate-800">{t('admin.regularUser', 'Regular User')}</option>
                  <option value="admin" className="bg-slate-800">{t('admin.admin', 'Admin')}</option>
                </select>
              </div>
            </div>
