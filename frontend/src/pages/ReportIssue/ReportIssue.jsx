...
export default function ReportIssue() {
  ...
  const categories = [
    ...
    { value: "waste", label: t('categories.waste') },
  ...
  const searchLocation = async (query) => {
    ...
    try {
      ...
      console.error("Search error:", err);
      setError(t('reportIssue.searchError', "Failed to search location")); // UPDATED
    } finally {
      setIsSearching(false);
    }
  };
  ...
  const handleFileUpload = (e) => {
    ...
    if (formData.images.length + files.length > maxFiles) {
      setError(t('reportIssue.maxImagesError', `You can only upload up to ${maxFiles} images`)); // UPDATED
      return;
    }

    const oversizedFiles = files.filter((file) => file.size > maxSize);
    if (oversizedFiles.length > 0) {
      setError(t('reportIssue.fileSizeError', "Some files are too large. Maximum size is 10MB per image.")); // UPDATED
      return;
    }
  ...
  const validateForm = () => {
    if (!formData.title.trim()) {
      setError(t('reportIssue.titleRequired', "Please enter an issue title")); // UPDATED
      return false;
    }
    if (!formData.category) {
      setError(t('reportIssue.categoryRequired', "Please select a category")); // UPDATED
      return false;
    }
    if (!formData.description.trim()) {
      setError(t('reportIssue.descriptionRequired', "Please provide a description")); // UPDATED
      return false;
    }
    if (!mapPosition) {
      setError(t('reportIssue.locationRequired', "Please search and select a location")); // UPDATED
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!userId) {
        setError(t('reportIssue.loginRequired', "User not authenticated. Please login again.")); // UPDATED
        setIsSubmitting(false);
        navigate("/login");
        return;
      }
      ...
      console.error("Error creating issue:", error);
      setError(error.message || t('reportIssue.submitError', "Failed to submit issue. Please try again.")); // UPDATED
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const getCurrentLocation = () => {
    if (!navigator.geolocation) {
      setError(t('reportIssue.geolocationNotSupported', "Geolocation is not supported by your browser.")); // UPDATED
      return;
    }

    setError(t('reportIssue.gettingLocation', "Getting your location...")); // UPDATED
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        console.error("Location error:", err);
        setError(t('reportIssue.locationError', "Unable to get your location.")); // UPDATED
      }
    );
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white/15 backdrop-blur-md rounded-2xl p-8 border border-white/20 shadow-lg">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div>
              <label className="block text-white text-sm font-semibold mb-2">
                {t('reportIssue.issueTitle')} <span className="text-red-400">{t('reportIssue.required')}</span>
              </label>
              <input
                type="text"
                name="title"
            ...
            <div>
              <label className="block text-white text-sm font-semibold mb-2">
                {t('reportIssue.category')} <span className="text-red-400">{t('reportIssue.required')}</span>
              </label>
              <select
                name="category"
                value={formData.category}
            ...
            <div>
              <label className="block text-white text-sm font-semibold mb-2">
                {t('reportIssue.description')} <span className="text-red-400">{t('reportIssue.required')}</span>
              </label>
              <textarea
                name="description"
                value={formData.description}
            ...
            <div>
              <label className="block text-white text-sm font-semibold mb-2">
                {t('reportIssue.searchLocation')} <span className="text-red-400">{t('reportIssue.required')}</span>
              </label>
              <div className="relative">
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      value={searchQuery}
                      onChange={(e) => setSearchQuery(e.target.value)}
                      placeholder={t('reportIssue.searchLocationPlaceholder')}
                      className="w-full pl-10 pr-4 py-3 bg-white rounded-lg text-gray-900 border border-gray-300 focus:outline-none focus:ring-2 focus:ring-[#00b4db] placeholder-gray-500"
                    />
                  </div>
                  <button
                  ...
                  >
                    <FaMapMarkerAlt />
                    <span className="hidden sm:inline">{t('reportIssue.gps')}</span>
                  </button>
                </div>
                ...
                {isSearching && (
                  <div className="absolute z-10 w-full mt-2 bg-white rounded-lg shadow-lg p-4 text-center">
                    <p className="text-sm text-gray-600">{t('reportIssue.searching')}</p>
                  </div>
                )}
              </div>
              ...
              {formData.location && (
                <div className="mt-2 p-3 bg-white/20 rounded-lg">
                  <p className="text-white text-sm">
                    <strong>{t('reportIssue.selected')}:</strong> {formData.location}
                  </p>
                  {mapPosition && (
                    <p className="text-white/70 text-xs mt-1">
                      {mapPosition.lat.toFixed(6)}, {mapPosition.lng.toFixed(6)}
            ...
            <div className="bg-white/15 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-lg flex flex-col">
              ...
              <p className="text-white/70 text-sm mt-3">
                {t('reportIssue.locationPreviewDesc')}
              </p>
            </div>

            <div>
              <label className="block text-white text-sm font-semibold mb-2">
                {t('reportIssue.images')}
              </label>
              <div className="border-2 border-dashed border-white/30 rounded-lg p-4 text-center">
                ...
                {formData.images.length === 0 ? (
                  <div>
                    ...
                    >
                      {t('reportIssue.chooseFiles')}
                    </button>
                    <p className="text-white/60 text-xs mt-2">{t('reportIssue.maxImages')}</p>
                  </div>
                ) : (
                  <div>
            ...
            <div className="flex items-center justify-center gap-4 mt-2">
              ...
              >
                {isSubmitting ? t('reportIssue.submitting') : t('reportIssue.submit')}
              </button>
            </div>
          </form>
        </div>
